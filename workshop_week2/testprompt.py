...
try:
    ...
    functions = [
        {
            ...
            'function': {
                ...
                'description': (
                    'Creates step-by-step instructions and tutorials ONLY when '
                    'users explicitly ask for procedural help or instructions. '
                    "Must contain clear action-oriented language like 'how to', "
                    "'steps to', 'guide me through', 'tutorial for', "
                    "'instructions to', 'walk me through', 'teach me to', "
                    "'show me how', 'create a guide', 'make a tutorial'. "
                    "NEVER use for: reviews, opinions, explanations, 'what is', "
                    "'tell me about', 'explain', 'describe', 'compare', "
                    "'review of', 'thoughts on', career advice, or general "
                ...
                'parameters': {
                    ...
                    'properties': {
                        ...
                        'category': {
                            'type': 'string',
                            'description': (
                                "Category of the guide (e.g., 'technical', "
                                "'setup', 'tutorial')"
                            ),
                        },
                        'difficulty_level': {
                        ...
                        'prerequisites': {
                            ...
                            'items': {'type': 'string'},
                            'description': (
                                'Prerequisites needed before following '
                                'the guide'
                            ),
                        },
                        'guide_steps': {
        ...
        },
        {
            ...
            'function': {
                ...
                'description': (
                    'Generates sample, test, or mock data when users request '
                    'data examples, want to create fake data, need sample '
                    'datasets, or ask for data generation. Automatically '
                    'triggered by questions containing words like: mock data, '
                    'sample data, generate data, test data, fake data, dummy '
                    'data, create data, data examples, sample users, example '
                    'products. Only include optional parameters if the user '
    ...
    def generate_mock_data(
        data_type, output_format=None, count=None, fields=None, locale=None
    ):
        """
        ...
        print(
            f'🔍 Generating mock data for type: {data_type} with count: '
            f'{count}'
        )
        ...
        if (
            ...
            print(
                f"🤖 Getting OpenAI model recommendations for data type: '{data_type}'"
            )
            enhanced_model, recommended_fields = (
                _get_openai_model_recommendations(data_type)
            )
        else:
        ...
        mock_data = {
            ...
            'user_provided_fields': bool(
                fields
            ),  # True if user provided specific fields
            'locale': locale,
            'generated_data': [],
            'ai_enhanced': bool(recommended_fields)
            and not bool(
                fields
            ),  # True only if AI-recommended fields were used
        }
        ...
        print(
            f'📊 Generated {count} {data_type} entries in {output_format.upper()} format{model_info}'
        )
        return mock_data
    ...
    def _generate_data_by_type(data_type, data_model, count, fields, locale):
        ...
        if fields and len(fields) > 0:
            print(
                f'🎯 Using AI-recommended fields: {", ".join(fields[:5])}{"..." if len(fields) > 5 else ""}'
            )
            return _generate_enhanced_data_with_fields(
        ...
        generator = data_generators.get(
            data_type.lower(), _generate_custom_data
        )
    ...
    def _get_openai_model_recommendations(data_type):
        ...
        try:
            ...
            print(
                f'🤖 AI found {len(recommended_models)} model options '
                f"for '{data_type}':"
            )
            for i, model in enumerate(recommended_models, 1):
                print(
                    f'   {i}. {model.get("model_name", "Unknown")} - {model.get("description", "No description")}'
                )
            ...
            print(
                f'🎯 Using model: {selected_model.get("model_name", "Default")}'
            )
            ...
            fallback = basic_models.get(
                data_type.lower(), basic_models['custom']
            )
            ...
            employees.append(
                {
                    ...
                    'manager_id': random.randint(1, max(1, i))
                    if i > 0
                    else None,
    ...
    def handle_standard_faq(
        question, answer, category='general', related_topics=None
    ):
    ...
    def format_guide_response(guide_data):
        ...
        response = (
            f'📚 === GUIDE: {guide_data.get("title", "Untitled")} ===\n\n'
        )
    ...
    def format_mock_data_response(mock_data):
        ...
        if format_type == 'json':
            ...
            response += _format_as_xml(
                data, mock_data.get('data_type', 'item')
            )
        ...
        try:
            ...
            if response.choices[0].message.tool_calls:
                ...
                if structured_response:
                    ...
                    print(
                        f'💾 Session stored. Total interactions: {len(faq_sessions)}'
                    )
    ...
    while True:
        ...
        readable_response, structured_response = (
            process_faq_with_function_calling(user_question)
        )
