...
def render_text_mode_ui(chat_history: List, handle_user_input_callback: Callable):
    ...
    with chat_container:
        if chat_history:
            for i, (user_msg, bot_msg, structured_data) in enumerate(chat_history):
...
def render_voice_mode_ui(chat_history: List, voice_interface, tts_interface, handle_voice_input_callback: Callable):
    ...
    if st.session_state.get('is_recording', False):
        st.markdown('<div class="voice-status">🎤 Listening... Speak naturally!</div>', unsafe_allow_html=True)
    elif st.session_state.get('is_processing', False):
        st.markdown('<div class="voice-status">🔄 Processing your voice...</div>', unsafe_allow_html=True)
    elif st.session_state.get('is_speaking', False):
        st.markdown('<div class="voice-status">🔊 Maya is speaking...</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="voice-status">🎙️ Voice mode active - automatic speech detection</div>', unsafe_allow_html=True)
    
    # Show last transcription if available
    if 'last_transcription' in st.session_state and st.session_state.last_transcription:
        st.markdown(f'<div class="voice-status" style="color: #28a745;">Last heard: "{st.session_state.last_transcription}"</div>', unsafe_allow_html=True)
    ...
    if st.session_state.get('is_speaking', False):
        ...
        with col2:
            if st.button("🔇 Stop Speaking", key="stop_speaking", use_container_width=True):
                if tts_interface:
    ...
    # Auto-recording info
    st.markdown("---")
    st.info("� Voice mode is active - speak naturally and your voice will be automatically detected and transcribed")
    ...
    if chat_history:
        ...
        with st.expander("View conversation history", expanded=False):
            for i, (user_msg, bot_msg, structured_data) in enumerate(chat_history[-5:]):  # Show last 5 exchanges
                st.markdown(f"**You:** {user_msg}")
                st.markdown(f"**Maya:** {bot_msg[:200]}{'...' if len(bot_msg) > 200 else ''}")
                st.markdown("---")
...
def render_mode_selector():
    ...
    mode = st.sidebar.radio(
        ...
        options=["text", "voice"],
        format_func=lambda x: "💬 Text Mode" if x == "text" else "🎤 Voice Mode",
        key="interface_mode"
    )
        ...
        **Voice Mode Features:**
        ...
        if st.sidebar.button("🎤 Test Microphone"):
            ...
            if check_microphone_permission():
                ...
                st.sidebar.markdown("Please enable microphone access in your browser settings.")
